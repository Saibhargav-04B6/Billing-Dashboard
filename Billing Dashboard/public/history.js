async function fetchData() {
    try {
        const response = await fetch('/data/billsover');
        if (!response.ok) {
        ...
        data.forEach(user => {
            ...
            <div class="editbtns"><button onclick="editdata(${user.id},'${user.bill_number}')">Edit</button>
            <button onclick="deletedata(${user.id})">Delete</button>`
           d.appendChild(e); 
           a=a+user.amount;    
           });
           var q=document.createElement('div');
           q.classList.add('total');
           q.innerHTML=`<b>Total_payments=<b>${a}`;
           d.appendChild(q);
